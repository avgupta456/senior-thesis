...
def sample_subgraphx(model, x, edge_index, node_idx_1, node_idx_2, T):
...
if __name__ == "__main__":
    ...
    output_2 = sample_subgraphx(model, x, edge_index, node_idx_1, node_idx_2, 2)
    output_2 = sorted(output_2.items(), key=lambda x: -x[1])

    output_5 = sample_subgraphx(model, x, edge_index, node_idx_1, node_idx_2, 5)
    output_5 = sorted(output_5.items(), key=lambda x: -x[1])

    output_10 = sample_subgraphx(model, x, edge_index, node_idx_1, node_idx_2, 10)
    output_10 = sorted(output_10.items(), key=lambda x: -x[1])

    output_20 = sample_subgraphx(model, x, edge_index, node_idx_1, node_idx_2, 20)
    ...
    for k in range(neighbors.shape[0] + 1):
        for label, output in zip(
            ["2", "5", "10", "20"], [output_2, output_5, output_10, output_20]
        ):
    ...
    for result, result_label in zip(
        ...
        fig, ax = plt.subplots()
        for key, label in zip(["2", "5", "10", "20"], ["2", "5", "10", "20"]):

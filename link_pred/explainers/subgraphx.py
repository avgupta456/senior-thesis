    ...
    def explain_edge(self, node_idx_1, node_idx_2):
        ...
        for neighbor in neighbors:
            ...
            for _ in range(self.T):
                ...
                old_pred = self.pred_model.decode(old_z, torch.tensor([[node_idx_1], [node_idx_2]]))
                
                fake_x[neighbor] = temp_x[neighbor]
                new_z = self.pred_model.encode(fake_x, self.edge_index)
                new_pred = self.pred_model.decode(new_z, torch.tensor([[node_idx_1], [node_idx_2]]))
                
                pred_diff = (new_pred - old_pred)
                pred_diffs.append(pred_diff.item())

            diff_avg, diff_std = sum(pred_diffs) / len(pred_diffs), statistics.stdev(pred_diffs) / np.sqrt(self.T)
